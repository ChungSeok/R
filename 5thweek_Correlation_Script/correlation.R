...
##패키지 설치(없을경우)
install.packages("Hmisc") # 기술통계 패키지
??Hmisc
install.packages("ggm") # Graphical Markov Models with Mixed Graphs
??ggm
install.packages("ggplot2") # 그래프 그리기 패키지
??ggplot2
install.packages("polycor") # 상관계수 패키지
??ploycor
...
examData = read.delim("5thweek_Correlation_Script/Exam Anxiety.dat",  header = TRUE)
examData
str(examData)
...
# 상관계수 r 이 -0.44가 나왔고,
# t검정통계량은 -4.938로
# 왼쪽꼬리부분의 임계점인 t(0.025,101)=-0.584보다 더 작기 때문에
# 95% 신뢰수준에서 모델은 유의함
# -0.4409934*sqrt(101/(1-(0.4409934^2)) = -4.938 = 상관계수의 유의성 검정공식


## 스피어만 상관분석-변수의 척도가 서열척도인 경우

liarData = read.delim("5thweek_Correlation_Script/The Biggest Liar.dat",  header = TRUE)
liarData
str(liarData)
